...
local function neutralizeFadeAnimation (group)
  if (not group) then return end
  group:Stop();
  -- Make the animation a no-op (animate from 1 to 1 instantly)
  -- so replays by SexyMap/LibDBIcon do nothing.
...
local function brightenButtonTextures (button)
  -- Neutralize fade animations from SexyMap and LibDBIcon
  -- that persist after reparenting buttons away from the Minimap.
  neutralizeFadeAnimation(button.sexyMapFadeOut);
  neutralizeFadeAnimation(button.fadeOut);
  button.showOnMouseover = false; -- Prevent LibDBIcon global fade loops
...
addon.import('Core/Events').registerEvent('PLAYER_LOGIN', function ()
  ...
  -- Re-brighten after SexyMap/LibDBIcon have attached their fade animations
  -- (SexyMap's StartFrameGrab fires at t+1, so t+2 ensures we run after it)
  executeAfter(2, function ()
    for _, button in ipairs(collectedButtons) do
      brightenButtonTextures(button);
    end
  end);
